...
export default function EconomicCalendarPanel({ assets = [], variant = 'full' }: { assets?: Asset[], variant?: 'full' | 'compact' } = {}) {
  ...
  if (variant === 'compact') {
    return (
      <div className="rounded-lg border border-border bg-card/50 backdrop-blur-sm overflow-hidden animate-fade-in transition-all hover:bg-card">
        <div className="px-3 py-1.5 flex items-center gap-3 overflow-hidden">
          <div className="flex items-center gap-1.5 shrink-0 border-r border-border pr-3">
            <CalendarClock className="h-3.5 w-3.5 text-primary" />
            <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground hidden sm:inline">Eco</span>
          </div>

          <div className="flex-1 overflow-x-auto no-scrollbar flex items-center gap-4 py-0.5">
            {sorted.length > 0 ? (
              sorted.slice(0, 5).map(ev => {
                const info = enrichedById.get(ev.id);
                if (!info) return null;
                return (
                  <button
                    key={ev.id}
                    onClick={() => setModalEvent(ev)}
                    className="flex items-center gap-2 whitespace-nowrap group transition-opacity hover:opacity-80"
                  >
                    <span className="text-[10px] font-mono text-muted-foreground">{info.time}</span>
                    <span className="text-[10px] font-bold">{info.flag}</span>
                    <span className="text-[11px] font-medium max-w-[120px] truncate">{ev.title}</span>
                    <span className={`h-1.5 w-1.5 rounded-full ${info.meta.dot}`} />
                  </button>
                );
              })
            ) : (
              <span className="text-[10px] text-muted-foreground">Nenhum evento relevante hoje</span>
            )}
          </div>
          ...
          <div className="flex items-center gap-1.5 shrink-0 border-l border-border pl-3">
            <div className="flex items-center gap-0.5">
              {(['high', 'medium', 'low'] as ImpactKey[]).map(k => (
                <div key={k} className={`h-1.5 w-1.5 rounded-full ${impactMeta[k].dot} ${filters.impacts.includes(k) ? 'opacity-100' : 'opacity-20'}`} />
              ))}
            </div>
            <button 
              onClick={() => load(true)} 
        ...
        {modalEvent && (
          <Suspense>
            <EconomicCalendarEventModal
              ...
              hasAlert={!!alerts[modalEvent.id]}
              onTickerAlert={createTickerAlert}
              assets={assets}
            />
          </Suspense>
        )}
  ...
  return (
    <div className="rounded-lg border border-border bg-card overflow-hidden animate-fade-in transition-all">
