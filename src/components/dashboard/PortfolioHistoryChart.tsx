...
export default function PortfolioHistoryChart({ snapshots, loading }: Props) {
  ...
  if (loading) {
    return (
      <div className="rounded-lg border border-border bg-card p-5 animate-fade-in flex items-center justify-center h-[400px]">
    ...
    return (
      <div className="rounded-lg border border-border bg-card p-5 animate-fade-in">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-lg font-semibold">Histórico Patrimonial</h2>
        </div>
          ...
          <AreaChart data={data} margin={{ top: 4, right: 4, left: 4, bottom: 0 }}>
            ...
            <Tooltip
              ...
              formatter={(value: number, name: string) => {
                if (name === 'total_value') return [formatCurrency(value), 'Patrimônio'];
                if (name === 'total_cost') return [formatCurrency(value), 'Custo'];
