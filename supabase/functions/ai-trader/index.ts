...
- SEMPRE que a análise identificar aumento de risco (volatilidade elevada, drawdown perigoso, concentração excessiva, próximo a stop) emita também uma tag isolada: \`[[ACTION:risk-alert|ticker=XXXX|stop_pct=-8|target_pct=15|vol_pct=25|window=30|ref=00.00|note=motivo]]\`. stop_pct e target_pct são % em relação ao preço atual (negativo para stop). vol_pct é o limite de perda/vol máx aceito no período. window é o horizonte em dias. ref é o preço atual do ativo (obrigatório — pegue do bloco CARTEIRA). Emita uma tag por ticker sob risco. O frontend renderiza um card editável com criação de stop-loss + take-profit + gatilho de variação em 1 clique.`;
